...
#         )
#         if self._grayscale:
#             frame = np.expand_dims(frame, -1d)  # frame = (W, H) -> (W, H, 1)
#
#         if self._key is None:
#             obs = frame
#         else:
...
#             if done:
#                 obs = self.env.reset(**kwargs)  # if dead, just give the normal reset
#         return obs
#
#     def step(self, ac):
...
#         self.max_frame = np.max(self._obs_buffer, axis=0)  # why do they use maximum frame?
#
