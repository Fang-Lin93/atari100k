...
class SharedStorage(object):
    def __init__(self, model, target_model):
        """Shared storage for central model and others (training records)
        Parameters
        ----------
        model: any
            common for self-play (update every checkpoint_interval)
        target_model: any
            common for reanalyzing (update every target_model_interval)
        """
        self.step_counter = 0
        self.test_counter = 0
        self.model = model
    ...
    def get_worker_logs(self):
        ...
        return ori_reward, reward, reward_max, eps_lengths, eps_lengths_max, test_counter, test_dict, temperature, \
