...
from .model import BaseNet, renormalize
...
def mlp(
    ...
    input_size: int
        dim of inputs
    layer_sizes: list
        dim of hidden layers, e.g. = [512, 512]
    output_size: int
        dim of outputs
    init_zero: bool
...
def conv3x3(in_channels, out_channels, stride=1):
    # conv keeps the size of the figure.
    return nn.Conv2d(
...
class DownSample(nn.Module):
    def __init__(self, in_channels, out_channels, momentum=0.1):
        ...
        self.conv1 = nn.Conv2d(
            ...
            out_channels // 2,
            kernel_size=3,
            stride=2,  # here the size is reduced
            padding=1,
            bias=False,
        )
...
class DynamicsNetwork(nn.Module):
    def __init__(
            ...
            fc_reward_layers,
            full_support_size,  # used for value support !
        ...
        self.fc = mlp(self.lstm_hidden_size, fc_reward_layers, full_support_size, init_zero=init_zero,
                      momentum=momentum)  # here is distributional reward, not the scalar reward !!

    def forward(self, x, reward_hidden):
        """
        Here reward_hidden is the hidden (ct, ht) of LSTM
        """
        state = x[:, :-1, :, :]  #TODO
        x = self.conv(x)
        x = self.bn(x)
        ...
        value_prefix = F.relu(value_prefix)
        value_prefix = self.fc(value_prefix)  # value prefix is the distribution.  # reward_hidden is tuple (c, h)
...
class PredictionNetwork(nn.Module):
    ...
    def forward(self, x):
        # share the same residual blocks
        for block in self.resblocks:
            x = block(x)
