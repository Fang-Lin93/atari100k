...
from eff.core.replay_buffer import ReplayBuffer
from eff.core.storage import SharedStorage, QueueStorage
from eff.core.selfplay_worker import DataWorker
...
def update_weights(model, batch, optimizer, replay_buffer, config, scaler, vis_result=False):
    """update common given a batch data
    Parameters
    ----------
    model: Any
        EfficientZero common
    batch: Any
    ...
    obs_batch = obs_batch_ori[:, 0: config.num_stack_obs * config.image_channel, :, :]
    ...
    if config.amp_type == 'torch_amp':
        ...
        with autocast():
            for step_i in range(config.num_unroll_steps):
                ...
                end_index = config.image_channel * (step_i + config.num_stack_obs)
...
def _train(model, target_model, replay_buffer, shared_storage, batch_storage, config, summary_writer):
    ...
    model: Any
        EfficientZero common
    target_model: Any
        EfficientZero common for reanalyzing
    replay_buffer: Any
        replay buffer
    shared_storage: Any
        model storage
...
def train(game_config, summary_writer, model_path=None):
    """training process
    Parameters
    ----------
    summary_writer: Any
    ...
    # reanalyze workers
    cpu_workers = [BatchWorker_CPU.remote(idx, replay_buffer, storage, batch_storage, mcts_storage, game_config) for idx in range(game_config.cpu_actor)]
    workers += [cpu_worker.run.remote() for cpu_worker in cpu_workers]
    gpu_workers = [BatchWorker_GPU.remote(idx, replay_buffer, storage, batch_storage, mcts_storage, game_config) for idx in range(game_config.gpu_actor)]
    workers += [gpu_worker.run.remote() for gpu_worker in gpu_workers]

    # self-play workers
    data_workers = [DataWorker.remote(rank, replay_buffer, storage, game_config) for rank in range(0, game_config.num_actors)]
    workers += [worker.run.remote() for worker in data_workers]
    # test workers
    workers += [_test.remote(game_config, storage)]

    # training loop
    final_weights = _train(model, target_model, replay_buffer, storage, batch_storage, game_config, summary_writer)
